...
.login {}
.login label {display: inline;padding: 10px 10px 10px 5px; height:30px; line-height: 30px; }
.login input {display: inline;width: 200px; height:28px; line-height: 30px;background-color: #fff; }
...
.login label span { text-align:right !important;color: #4d7284;text-shadow: 0 1px 0 #ffffff;width: 160px;display: inline-block;}
...
@media (max-width:480px) {
  ...
  .login input {display: inline;width: 120px; height:24px; line-height: 30px; background-color: #fff;}
      ...
      return false;
		...
		$.post("<?php echo iPHP_SELF; ?>",param,function(json){
          ...
          if(json.msg){
            ...
            iCMS.alert("Неверное имя пользователя или пароль!");
          }
				}
		},'json');
		return false;
  ...
  <div class="iCMS_login">
    ...
    <div class="login">
      <form action="<?php echo iPHP_SELF; ?>" method="post" enctype="multipart/form-data" class="form-horizontal" id="iCMS-Login" target="iPHP_FRAME">
        <div class="ipt_uname">
          <label for="username"><i></i><span>Имя пользователя</span></label>
          <input type="text" name="username" id="username" />
        </div>
        <div class="clear"></div>
        <div class="ipt_pass">
          <label for="password"><i></i><span>Пароль</span></label>
          <input type="password" name="password" id="password" />
        </div>
        <div class="clear"></div>
        <div class="ipt_seccode">
          <label for="seccode"><i></i><span>Защитный код</span></label>
          <input type="text" name="seccode" id="seccode" class="iCMS_seccode">
          <img src="<?php echo iPHP_SELF; ?>?do=seccode" alt="Защитный код" class="iCMS_seccode_img r3"/>
          <a href="javascript:;" class="iCMS_seccode_text">Обновить</a>
        </div>
        <div class="clear"></div>
        <button class="btn btn-small btn-primary" type="submit"><i class="icon-ok icon-white"></i>Войти</button>
      </form>
    </div>
  </div>
</div>
