...
public static function etc($app,$name='router',$data=null){
    ...
    public static function get_package($name,$dir,$REMOVE_PATH=null) {
        ...
        if($REMOVE_PATH){
            $v_list = $zip->create($lists,PCLZIP_OPT_REMOVE_PATH, $REMOVE_PATH); //Сжатие файлов
        }else{
            $v_list = $zip->create($lists); //Сжатие файлов
        }
        $v_list == 0 && iPHP::error_throw($zip->errorInfo(true)); //Если есть ошибка, отображается сообщение об ошибке..
        return $package;
    }
