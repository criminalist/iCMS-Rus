...
class commentAdmincp{
    ...
    public function do_iCMS($appid=0){
        ...
        iUI::pagenav($total,$maxperpage,"Комментарии к посту");
    ...
    /**
     * [Просмотреть комментарий ответить]
     * @return [type] [description]
     */
    public function do_get_reply(){
    	$this->id OR exit("Выберите комментарий");
        $comment = iDB::row("SELECT * FROM `#iCMS@__comment` WHERE `id`='$this->id' LIMIT 1");
        empty($comment) && exit('<div class="claerfix mb10"></div>Комментарий был удален');
    ...
    public function do_del($id = null,$dialog=true){
    	$id===null && $id=$this->id;
    	$id OR iUI::alert('Выберите комментарий для удаления!');
    	$rs = $this->get($id);
        $this->del($rs);
        $dialog && iUI::success('Комментарий успешно удален','js:parent.$("#id-'.$id.'").remove();');
    }

    public function do_batch(){
        list($idArray,$ids,$batch) = iUI::get_batch_args("Выберите комментарий");
    	switch($batch){
    		case 'dels':
				iUI::$break	= false;
	    		...
	    		iUI::$break	= true;
				iUI::success('Комментарии успешно удалены!','js:1');
    		break;
		}
	}
