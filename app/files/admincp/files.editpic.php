...
<div id="onBrowse" class="well" style="display:none;">
  <a class="btn btn-success" href="<?php echo __ADMINCP__; ?>=files&do=picture&from=modal&click=file&callback=brofile" data-toggle="modal" data-meta='{"width":"75%","height":"480px"}' data-zIndex="9999999" title="Выберите картинку с сайта"><i class="fa fa-picture-o"></i> Выбрать на сервере</a>
  <hr />
  <a id="local" class="btn btn-primary"><i class="fa fa-upload"></i> Выбрать с компьютера</a>
  <input id="localfile" type="file" multiple="multiple" accept="image/*" class="hide"/>
</div>
<div id="onUpload" class="well" style="display:none;">
  <a class="btn btn-success" href="<?php echo __ADMINCP__; ?>=files&do=picture&from=modal&click=dir&callback=savetodir" data-toggle="modal" data-meta='{"width":"75%","height":"480px"}' data-zIndex="9999999" title="Сохранить в новом каталоге"><i class="fa fa-save"></i>Сохранить в</a>
  ...
  <div class="input-prepend input-append">
    ...
    <div class="switch" data-on-label="保留" data-off-label="Закрыть">
  ...
  <div class="iCMS-container">
    <div id="PhotoEditor">
      <?php if(!is_array($src)){ ?>
      <img src="<?php echo $src;?>" alt="Предварительный просмотр">
      <?php } ?>
    </div>
</div>
...
//console.log(sel_channel);
    ...
    xiuxiu.setUploadURL("<?php echo ACP_HOST;?><?php echo __ADMINCP__; ?>=files&do=IO&format=json&id=<?php echo $file_id;?>");
...
//xiuxiu.loadImages(images, {"loadImageChannel":channel});
//xiuxiu.uploadFail();
 ...
 //console.log(sel_dialog);
    ...
    xiuxiu.onBeforeUpload = function (data, id){
      ...
      if(size ><?php echo $max_size;?>) {
          iCMS.alert("Размер изображения не должен превышать <?php echo $this->upload_max_filesize;?>");
         return false;
      }
      return true;
    }
...
function loading(){
  //sel_dialog.content('<div class="iPHP-msg"><img src="./app/admincp/ui/img/ajax_loader.gif" /> <span class="label label-inverse">Загрузка изображения... подождите!</span></div>');
}
