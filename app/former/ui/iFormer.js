var iFormer = {
            ...
            switch (obj_type) {
                ...
                case 'seccode':
                    ...
                    var div_after = function () {
                        ...
                        span.append(
                            '<img src="'+iCMS.config.API+'?do=seccode" alt="Защитный код" class="seccode-img r3">'+
                            '<a href="javascript:;" class="seccode-text">Обновить</a>'
                        );
                        $div.addClass('input-append');
                        $div.append(span);
                    }
    ...
    hidden:function ($elem,$div,$span) {
        var parent = iFormer.widget('span').addClass('add-on');
        var info = iFormer.widget('span').addClass('label label-info').text('Спойлер');
        if($span){
            parent.append($span);
        }
        parent.append(info);
    ...
    edit_dialog: function(obj, callback) {
        ...
        return iCMS.dialog({
            ...
            ok: function() {
                ...
                if(dname.length){
                    iCMS.alert("Поле с таким названием уже существует, введите другое");
                    return false;
                }
