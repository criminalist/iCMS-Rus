...
class formsAdmincp{
   ...
   */
    public function do_save(){
        ...
        if(empty($id)) {
            ...
            if($addons_fieldata){
              ...
              if($create){
                iDB::check_table($addons_name) && iUI::alert('['.$addons_name.'] дополнительная таблица уже существует!');
              }
            }
            ...
            iDB::value("SELECT `id` FROM `#iCMS@__forms` where `app` ='$app' AND `id` !='$id'") && iUI::alert('Форма уже существует!');
    ...
    public function do_batch(){
        list($idArray,$ids,$batch) = iUI::get_batch_args("Выберите таблицы单");
        switch($batch){
          case 'data-dels':
            iUI::$break = false;
