...
class htmlAdmincp{
    ...
    public function CreateIndex($indexTPL,$indexName,$p=1,$loop=1){
		...
		$msg    = "Всего <span class='label label-info'>{$_total}</span>页 已生成<span class='label label-info'>".$GLOBALS['page']."</span>页,";
...
//		$surplus		= ceil($total-$p);
		...
		$msg.="用时<span class='label label-info'>{$use_time}</span>секунд";
		$query["alltime"] = $this->alltime+$use_time;
		$loopurl = $this->loopurl($total,$query);
		if($loopurl){
			$moreBtn = array(
				array("id"=>"btn_stop","text"=>"Остановить","url"=>APP_URI."&do=index"),
				array("id"=>"btn_next","text"=>"Продолжить","src"=>$loopurl,"next"=>true)
	        );
			$dtime    = 1;
			$all_time = $looptimes*$use_time+$looptimes+1;
			$msg.="<hr />预计全部生成还需要<span class='label label-info'>{$all_time}</span>секунд";
        }else{
			$moreBtn = array(
				array("id"=>"btn_next","text"=>"Завершить","url"=>APP_URI."&do=index")
	        );
			$dtime = 5;
	        $msg.="<hr />Все статические страницыы успешно сгенерированы<hr />Общее время потраченное на генерацию<span class='label label-info'>".$query["alltime"]."</span>секунд";
        }
...
//		$surplus		= ceil($total-$p);
		...
		if($loopurl){
			$moreBtn = array(
				array("id"=>"btn_stop","text"=>"Остановить","url"=>APP_URI."&do=category"),
				array("id"=>"btn_next","text"=>"Продолжить","src"=>$loopurl,"next"=>true)
	        );
			$dtime    = 1;
			$all_time = $looptimes*$use_time+$looptimes+1;
			$msg.="<hr /><span class='label label-success'>{$name}</span>栏目,预计全部生成还需要<span class='label label-info'>{$all_time}</span>секунд";
        }else{
			$moreBtn = array(
				array("id"=>"btn_next","text"=>"Завершить","url"=>APP_URI."&do=category")
	        );
			$dtime = 3;
	        $msg.="<hr /><span class='label label-success'>{$name}</span>栏目,已全部生成完成.Общее время потраченное на генерацию<span class='label label-info'>".$query["alltime"]."</span>секунд";
        	if($k<$len){
				$query["k"]       = $k+1;
				$query["alltime"] = 0;
				$GLOBALS['page']  = 0;
    ...
    public function do_createArticle($aid=null){
		...
		if($aid){
			...
			iUI::success($title.'<hr />Генерация статики завершена!');
		}
        if($category[0]=='all'){
            category::$appid = iCMS_APP_ARTICLE;
        ...
        $GLOBALS['page']++;
		$use_time	= iPHP::timer_stop();
		$msg.="<hr />Время <span class='label label-info'>{$use_time}</span>секунд";
		$query["total_num"]	= $total;
		$query["alltime"]	= $this->alltime+$use_time;
		$loopurl	= $this->loopurl($looptimes,$query);
		if($loopurl){
			$moreBtn	= array(
				array("id"=>"btn_stop","text"=>"Остановить","url"=>APP_URI."&do=article"),
				array("id"=>"btn_next","text"=>"Продолжить","src"=>$loopurl,"next"=>true)
	        );
	        $dtime		= 1;
			$all_time	= $looptimes*$use_time+$looptimes+1;
			...
			$moreBtn	= array(
				array("id"=>"btn_next","text"=>"Завершить","url"=>APP_URI."&do=article")
	        );
	        $dtime		= 5;
	        $msg.="<hr />Все статические страницыы успешно сгенерированы<hr />Общее время потраченное на генерацию<span class='label label-info'>".$query["alltime"]."</span>секунд";
        }
