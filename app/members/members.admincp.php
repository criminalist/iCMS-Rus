...
class membersAdmincp{
    ...
    public function do_iCMS(){
        ...
        list($orderby,$orderby_option) = get_orderby(array(
            'uid'        =>"UID",
            'regtime'    =>"Дата и время регистрации",
            'logintimes' =>"登陆次数",
            'post'       =>"发表数",
        ));
    ...
    public function do_save(){
        ...
        if(empty($uid)) {
            iDB::value("SELECT `uid` FROM `#iCMS@__members` where `username` ='$username' LIMIT 1") && iUI::alert('Имя пользователя уже существует');
            ...
            iDB::value("SELECT `uid` FROM `#iCMS@__members` where `username` ='$username' AND `uid` !='$uid' LIMIT 1") && iUI::alert('Имя пользователя уже существует');
