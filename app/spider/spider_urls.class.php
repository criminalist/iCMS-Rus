        ...
        if($work=='shell'){
            ...
            if($project['psleep']){
                if(time()-$lastupdate<$project['psleep']){
                    echo date("Y-m-d H:i:s ").'Схема ['.$pid."]:".format_date($lastupdate)."刚采集过了,请".($project['psleep']/3600)."小时后在继续采集\n";
                    return;
                }
            }
            if($pid){
...
//      }
        ...
        foreach ($urlsArray AS $key => $url) {
            ...
            if(empty($urlsDataCount)){
                spider_error::log("URL существует 0!",$url,'url.zero',self::$ids,false);
                continue;
            }

            //PID@xx список возвращенных URL
            ...
            if($work=="shell"){
                ...
                foreach ($urlsData AS $lkey => $value) {
                    ...
                    echo date("Y-m-d H:i:s ")."\033[32mНачинается сбор 查看错误信息 (".$_index."/".$urlsDataCount.")\033[0m\n";
