...
$(function(){
  ...
  <?php } ?>
	...
	$(".edit").dblclick(function(){
		var a=$(this),tid=a.attr("tid"),box=$('#ed-box'),title=$.trim($('.aTitle',a).text());
		$('#edcid,#edpid').empty();
		...
		$.getJSON("<?php echo APP_URI; ?>",{'do':'getjson','id':tid},function(d){
...
<div class="iCMS-container">
    ...
    <div class="widget-content">
      <form action="<?php echo iPHP_SELF ; ?>" method="get" class="form-inline">
        ...
        <div class="input-prepend">
          <span class="add-on">专题属性</span>
          <select name="pid" id="pid" class="span2 chosen-select">
            <option value="-1">所有专题</option>
        ...
        <div class="input-prepend input-append">
          ...
          <span class="add-on tip" title="选中查询所有关联到此栏目的专题">
  ...
  <div class="widget-box" id="<?php echo APP_BOXID;?>">
    <div class="widget-title"> <span class="icon">
      ...
      <h5><?php if($cid){echo ' <span class="label label-info">'.category::get($cid)->name.'</span>';}?>专题列表</h5>
      <span title="总共<?php echo $total;?>篇专题" class="badge badge-info tip-left"><?php echo $total;?></span>
    </div>
          ...
          <tbody>
            <?php
                ...
                if($rs)foreach ($rs as $key => $value) {
                  ...
                  $iurl = iURL::get('topic',array($value,$C));
                  $value['url'] = $iurl->href;
            ?>
            <tr id="id<?php echo $value['id'] ; ?>">
              ...
              <td><div class="edit" tid="<?php echo $value['id'] ; ?>">
                ...
                <div class="row-actions">
                  <a href="<?php echo __ADMINCP__; ?>=files&indexid=<?php echo $value['id'] ; ?>&appid=<?php echo self::$appid;?>&method=database" class="tip-bottom" title="查看专题图片库" target="_blank"><i class="fa fa-picture-o"></i></a>
                  <a href="<?php echo APP_URI; ?>&do=findpic&id=<?php echo $value['id'] ; ?>" class="tip-bottom" title="查找专题所有图片" target="_blank"><i class="fa fa-picture-o"></i></a>
                  <?php if($value['postype']=="0"){ ?>
                  ...
                  <a href="<?php echo __ADMINCP__; ?>=comment&appname=topic&appid=<?php echo self::$appid;?>&iid=<?php echo $value['id'] ; ?>" class="tip-bottom" title="<?php echo $value['comments'] ; ?>条评论" target="_blank"><i class="fa fa-comment"></i></a>
                  <?php } ?>
                  <!-- <a href="<?php echo __ADMINCP__; ?>=chapter&tid=<?php echo $value['id'] ; ?>" class="tip-bottom" title="章节管理" target="_blank"><i class="fa fa-sitemap"></i></a> -->
                  <?php if($value['status']=="1"){ ?>
                  <?php if(apps::check('push')){ ?>
                  <a href="<?php echo __ADMINCP__; ?>=push&do=add&title=<?php echo $value['title'] ; ?>&pic=<?php echo $value['pic'] ; ?>&url=<?php echo $value['url'] ; ?>" class="tip-bottom" title="推送此专题"><i class="fa fa-thumb-tack"></i></a>
                  <?php } ?>
                  ...
                  <a href="<?php echo APP_URI; ?>&do=update&id=<?php echo $value['id'] ; ?>&_args=pubdate:now" class="tip-bottom" target="iPHP_FRAME" title="更新专题时间"><i class="fa fa-clock-o"></i></a>
                  <?php } ?>
                  <?php if($value['status']=="0"){ ?>
                  <a href="<?php echo APP_FURI; ?>&do=update&id=<?php echo $value['id'] ; ?>&_args=status:1" class="tip-bottom" target="iPHP_FRAME" title="发布专题"><i class="fa fa-share"></i></a>
                  <a href="<?php echo APP_URI; ?>&do=update&id=<?php echo $value['id'] ; ?>&_args=status:1,pubdate:now" class="tip-bottom" target="iPHP_FRAME" title="更新专题时间,并发布"><i class="fa fa-clock-o"></i></a>
                  <?php } ?>
                  <?php if($value['status']=="2"){ ?>
              ...
              <td><?php if($value['status']=="1"){ ?>
                ...
                <a href="<?php echo APP_FURI; ?>&do=update&id=<?php echo $value['id'] ; ?>&_args=status:2" target="iPHP_FRAME" class="del btn btn-danger btn-mini" title="移动此专题到回收站" />删除</a>
                <?php } ?>
                <?php if($value['status']=="2"){ ?>
                ...
                <div class="input-prepend input-append mt20">
                  ...
                  <div class="btn-group dropup" id="iCMS-batch"> <a class="btn dropdown-toggle" data-toggle="dropdown" tabindex="-1"><i class="fa fa-wrench"></i> 批 量 操 作 </a><a class="btn dropdown-toggle" data-toggle="dropdown" tabindex="-1"> <span class="caret"></span></a>
                    <ul class="dropdown-menu">
                      ...
                      <li class="divider"></li>
                      <li><a data-toggle="batch" data-action="prop"><i class="fa fa-puzzle-piece"></i> 设置专题属性</a></li>
                      ...
                      <li class="divider"></li>
                      <li><a data-toggle="batch" data-action="related"><i class="fa fa-sitemap"></i> 专题关联</a></li>
                      <li class="divider"></li>
