...
class topicFunc{
	public static function topic_list($vars) {
		...
		if (isset($vars['cids']) && !$vars['cid']) {
			...
			if ($cids) {
				iMap::init('category', iCMS_APP_TOPIC,'cid');
		...
		if (isset($vars['pids']) && !isset($vars['pid'])) {
			iMap::init('prop', iCMS_APP_TOPIC,'pid');
			$map_where += iMap::where($vars['pids']);
		}

		if (isset($vars['tids'])) {
			iMap::init('tag', iCMS_APP_TOPIC,'tags');
			$map_where += iMap::where($vars['tids']);
		}
		if (isset($vars['keywords'])) {
	//最好使用 iCMS:topic:search
		...
		if ($map_where) {
			...
			//$map_order_sql = " ORDER BY `icms_topic`.`id` $by";
			//
			$where_sql .= ' AND ' . $map_sql['where'];
			$where_sql = ",{$map_sql['from']} {$where_sql} AND `#iCMS@__topic`.`id` = {$map_table}.`iid`";
			//derived
		...
		if ($vars['page']) {
			...
			$total      = iPagination::totalCache("SELECT count(*) FROM `#iCMS@__topic` {$where_sql}", $total_type,iCMS::$config['cache']['page_total']);
			...
			iView::assign("topic_list_total", $total);
		}
		//随机特别处理
		if ($vars['orderby'] == 'rand') {
			$ids_array = iSQL::get_rand_ids('#iCMS@__topic', $where_sql, $maxperpage, 'id');
			if ($map_order_sql) {
				$map_order_sql = " ORDER BY `#iCMS@__topic`.`id` $by";
			}
		}
		$hash = md5(json_encode($vars) . $order_sql . $limit);
		if ($offset) {
			if ($vars['cache']) {
				$map_cache_name = iPHP_DEVICE . '/topic_page/' . $hash;
				$ids_array = iCache::get($map_cache_name);
			}
			if (empty($ids_array)) {
				$ids_order_sql = $map_order_sql ? $map_order_sql : $order_sql;
				$ids_array = iDB::all("SELECT `#iCMS@__topic`.`id` FROM `#iCMS@__topic` {$where_sql} {$ids_order_sql} {$limit}");
		...
		if ($ids_array) {
			...
			$ids = $ids ? $ids : '0';
			$where_sql = "WHERE `#iCMS@__topic`.`id` IN({$ids})";
			$limit = '';
		}
		if ($vars['cache']) {
			$cache_name = iPHP_DEVICE . '/topic/' . $hash;
			$resource = iCache::get($cache_name);
			if(is_array($resource)) return $resource;
		}
		// $func = 'topic_array';
		// if($vars['func']=="user_home"){ //暂时只有一个选项
		//     $func = '__topic_user_home_array';
		// }
		if (empty($resource)) {
			$resource = iDB::all("SELECT `#iCMS@__topic`.* FROM `#iCMS@__topic` {$where_sql} {$order_sql} {$limit}");
			$resource = topicFunc::topic_array($vars, $resource);
			$vars['cache'] && iCache::set($cache_name, $resource, $cache_time);
		}
		//print_r($resource);
		return $resource;
	}
	public static function topic_search($vars) {
		...
		if ($vars['page']) {
			$total = $res['total'];
			iView::assign("topic_search_total", $total);
			$pagenav = isset($vars['pagenav']) ? $vars['pagenav'] : "pagenav";
			$pnstyle = isset($vars['pnstyle']) ? $vars['pnstyle'] : 0;
			$multi = iPagination::make(array('total' => $total, 'perpage' => $maxperpage, 'unit' => iUI::lang('iCMS:page:list'), 'nowindex' => $GLOBALS['page']));
			$offset = $multi->offset;
		}
		$resource = iDB::all("SELECT * FROM `#iCMS@__topic` WHERE {$where_sql} {$order_sql} LIMIT {$maxperpage}");
		$resource = topicFunc::topic_array($vars, $resource);
		return $resource;
	}
	public static function topic_data($vars) {
		$vars['aid'] OR iUI::warning('iCMS&#x3a;topic&#x3a;data 标签出错! 缺少"aid"属性或"aid"值为空.');
		$data = iDB::row("SELECT body,subtitle FROM `#iCMS@__topic_data` WHERE aid='" . (int) $vars['aid'] . "' LIMIT 1;", ARRAY_A);
		topicApp::hooked($data);
		return $data;
	}
	public static function topic_prev($vars) {
		$vars['order'] = 'p';
		return topicFunc::topic_next($vars);
	}
	public static function topic_next($vars) {
		// if($vars['param']){
		//     $vars+= $vars['param'];
		//     unset($vars['param']);
		// }
		...
		$hash = md5($sql);
		if ($vars['cache']) {
			$cache = iPHP_DEVICE . '/topic/' . $hash;
			$array = iCache::get($cache);
		}
		if (empty($array)) {
			$rs = iDB::row("SELECT * FROM `#iCMS@__topic` WHERE `status`='1' {$sql}");
			if ($rs) {
				$category = categoryApp::get_cahce_cid($rs->cid);
				$array = array(
					'title' => $rs->title,
					'pic' => filesApp::get_pic($rs->pic),
					'url' => iURL::get('topic', array((array) $rs, $category))->href,
				);
			}
	...
	public static function topic_array($vars, $variable) {
		$resource = array();
		if ($variable) {
	        if($vars['data']||$vars['pics']){
	            $aidArray = iSQL::values($variable,'id','array',null);
	            $aidArray && $topic_data = (array) topicApp::data($aidArray);
	            unset($aidArray);
	        }
	        if($vars['meta']){
	            $aidArray = iSQL::values($variable,'id','array',null);
				$aidArray && $meta_data = (array)apps_meta::data('topic',$aidArray);
	            unset($aidArray);
	        }
			...
			foreach ($variable as $key => $value) {
				$value = topicApp::value($value, false, $vars);

				if ($value === false) {
					continue;
				}
	            if(($vars['data']||$vars['pics']) && $topic_data){
	                $value['data']  = (array)$topic_data[$value['id']];
	                if($vars['pics']){
