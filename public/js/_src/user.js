    ...
    $User = {
        ...
        widget: {
            loading: '<div class="tip_info">' +
                ...
                '<span> Информация о пользователе загружается ...</span>' +
                '</div>'
        }
    };
    return $.extend($User, {
        ...
        REPORT: function(a) {
            ...
            var $this = $(a),
                _title = $this.attr('title') || 'Хотите сообщить о нарушении?',
            ...
            $('[name="ok"]', box).click(function(event) {
                ...
                if (!data.reason) {
                    UI.alert("Выберите причину");
                    return false;
                }
                if (data.reason == "0") {
                    data.content = content.val();
                    if (!data.content) {
                        UI.alert("Напишите о нарушении");
                        return false;
                    }
                }
                data.action = 'report';
