...
if(empty($_SERVER['argv'][1])){
    exit("ERROR:need argc".PHP_EOL."exp:".$_SERVER['argv'][0]." [rid]".PHP_EOL.PHP_EOL.PHP_EOL);
}
iHttp::$CURLOPT_TIMEOUT        = 60; //数据传输的最大允许时间
iHttp::$CURLOPT_CONNECTTIMEOUT = 10;  //连接超时时间

$rids = explode(',', $_SERVER['argv'][1]);

$project = iDB::all("SELECT * FROM `#iCMS@__spider_project` WHERE `rid` IN('".implode("','", $rids)."') order by `id` ASC");
...
foreach ((array)$project as $key => $pro) {
    ...
    if(file_exists($pfile)){
        ...
        if($time-$pro['lastupdate']>=$pro['psleep']){
            unlink_pid();
        }else{
